...
To get started, may I please have your **name**, and are you looking for support for yourself or someone else?`,
  ...
  const handleSendMessage = async (e?: FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to get response from care assistant.');
      }
  ...
  const handleExtractDetails = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Could not parse conversation details.');
      }

      const extracted = await response.json();
      setExtractedData(extracted);
    } catch (err: any) {
      setError('Could not extract details. You can continue chatting or fill out the main form.');
      console.error(err);
    } finally {
    ...
    <div className={`fixed bottom-6 right-6 z-50 font-sans ${highContrast ? 'high-contrast' : ''}`}>
      ...
      {!isOpen && (
        ...
        >
          ...
          <span className="text-xs tracking-wide">Chat with Care AI ⚡</span>
        </button>
      )}
